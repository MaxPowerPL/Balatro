...
class ShaderBackground:
    def __init__(self, window_width, window_height):
        try:
            ...
            print(f"Błąd kompilacji shadera: {e}")
            raise

        self.time = 0.0
        # Prostokąt na cały ekran (-1 do 1)
        ...
        p = PALETTES[0]
        self.curr_c1 = list(p["c1"])
        self.curr_c2 = list(p["c2"])
        self.curr_c3 = list(p["c3"])

        self.target_c1 = list(p["c1"])
        self.target_c2 = list(p["c2"])
        self.target_c3 = list(p["c3"])

    def lerp_color(self, current, target, dt, speed=1.0):
        r = current[0] + (target[0] - current[0]) * dt * speed
        g = current[1] + (target[1] - current[1]) * dt * speed
        b = current[2] + (target[2] - current[2]) * dt * speed
        return [r, g, b]
    ...
    def update(self, dt, width, height):
        ...
        if self.palette_timer > self.switch_time:
            ...
            print(f"Zmiana motywu na: {new_p['name']}")
